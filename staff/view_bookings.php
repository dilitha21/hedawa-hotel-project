...
$bookings = $pdo->query("SELECT b.*, r.name as room_name, u.full_name, u.email FROM bookings b LEFT JOIN rooms r ON b.room_id = r.id JOIN users u ON b.user_id = u.id ORDER BY b.booking_date DESC, b.booking_time DESC")->fetchAll();
?>
<div class="staff-container" style="display: flex; min-height: calc(100vh - 80px - 200px); background: #f8fafc;">
    ...
    <aside style="width: 250px; background: var(--white); box-shadow: var(--shadow-sm); padding: 2rem 0;">
        ...
        <ul style="list-style: none;">
            ...
            <li><a href="view_bookings.php" style="display: block; padding: 1rem 2rem; color: var(--primary); font-weight: 500; background: var(--bg-color); border-right: 4px solid var(--primary);"><i class="fa-solid fa-book-open"></i> View Bookings</a></li>
    ...
    <div style="flex: 1; padding: 3rem; overflow-x: auto;">
        ...
        <?php if(isset($_GET['msg']) && $_GET['msg'] === 'deleted'): ?>
            <div style="background: var(--bg-color); border-left: 4px solid var(--primary); padding: 1rem; margin-bottom: 2rem;">
                <p style="color: var(--primary-dark); font-weight: 500; margin:0;">Booking successfully deleted.</p>
            </div>
        <?php endif; ?>
        ...
        <table style="width: 100%; border-collapse: collapse; background: var(--white); box-shadow: var(--shadow-sm); border-radius: var(--radius);">
            ...
            <tr style="border-bottom: 1px solid #edf2f7;">
                <td style="padding: 1rem; font-weight: 500;">#<?= $b['id'] ?></td>
                <td style="padding: 1rem;"><?= htmlspecialchars($b['full_name']) ?><br><small style="color: var(--text-muted);"><?= htmlspecialchars($b['email']) ?></small></td>
                <td style="padding: 1rem;"><?= htmlspecialchars($b['room_name'] ?? 'General') ?></td>
                <td style="padding: 1rem; color: var(--text-muted); font-size: 0.95rem;"><?= htmlspecialchars($b['booking_date']) ?> at <?= htmlspecialchars($b['booking_time']) ?></td>
                <td style="padding: 1rem;"><?= htmlspecialchars($b['guests']) ?></td>
                <td style="padding: 1rem;">
                    <span style="background: <?= $b['status'] == 'confirmed' ? 'var(--accent)' : '#edf2f7' ?>; color: var(--text-main); padding: 0.2rem 0.6rem; border-radius: 4px; font-size: 0.9rem;">
                ...
                <td style="padding: 1rem; text-align: center;">
                    <form action="../actions/booking_process.php" method="POST" style="margin:0;">
                        ...
                        <button type="submit" class="btn btn-outline" style="padding: 0.3rem 0.8rem; color: #e53e3e; border-color: #e53e3e; font-size: 0.85rem;" onclick="return confirm('WARNING: Unconditionally delete booking #<?= $b['id'] ?>?');"><i class="fa-solid fa-trash"></i> Delete</button>
                    </form>
                </td>
            </tr>
            <?php endforeach; ?>
